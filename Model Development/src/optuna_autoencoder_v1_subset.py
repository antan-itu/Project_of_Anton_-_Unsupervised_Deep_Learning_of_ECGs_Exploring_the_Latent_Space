...
N_ITERATIONS = 30                     # Optuna trials to run
...
GRID_LATENT_DIMS = [128, 256, 512]
GRID_LEARNING_RATES = [0.001, 0.0005]
GRID_BASE_FILTERS = [64, 128, 256]
GRID_KERNEL_SIZES = [5, 7, 9, 11, 13]
...
def objective(trial):
    # 1. Optuna dynamically picks the hyperparameters from our grid space
    ...
    # 2. Setup folders for this specific trial
    ...
    best_r2 = -float('inf')
    is_pruned = False # Tracker to see if the trial gets killed early
    ...
    try:
        ...
        for fold in range(K_FOLDS):
            ...
            history = autoencoder.fit(
                ...
                verbose=0 # Suppressed to keep logs clean
            )
            ...
            # Keep track of the best model for plotting
            ...
            # OPTUNA PRUNING: Tell Optuna how this fold did.
            trial.report(val_metrics['Val_R2'], step=fold)
            if trial.should_prune():
                is_pruned = True
                print("  [!] Trial pruned by Optuna due to poor early performance.")
                raise optuna.exceptions.TrialPruned() # This acts as the emergency eject button

    finally:
        # --- THIS BLOCK ALWAYS RUNS, EVEN IF PRUNED! ---
        tf.keras.backend.clear_session()
        
        # Only try to save if we actually completed at least 1 fold
        if len(fold_metrics_list) > 0:
            ...
            avg_metrics["Pruned"] = is_pruned # Note in the CSV if it was killed early

            print(f"  >>> Trial Finished. Average Val R2: {avg_metrics['Avg_Val_R2']:.4f} (±{avg_metrics['Std_Val_R2']:.4f})\n")

            with open(os.path.join(run_dir, "avg_metrics.json"), "w") as f:
                json.dump(avg_metrics, f, indent=4)

            # Only generate plots if we found a valid model
    ...
    # Optuna tries to MAXIMIZE whatever we return from this function
    return avg_metrics['Avg_Val_R2']

# 3. Create the Optuna Study and run it
...
# BONUS: Export Optuna's internal, master tracking database to a CSV as a backup!
study_df = study.trials_dataframe()
study_df.to_csv(os.path.join(OUTPUT_DIR, "optuna_master_history.csv"), index=False)
