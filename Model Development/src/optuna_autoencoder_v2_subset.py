...
def evaluate_overall_performance(model, dataset, eval_batches, prefix=""):
    ...
    # Calculate metrics and round them immediately to 3 decimal places
    ...
    try:
        ...
        if len(fold_metrics_list) > 0:
            ...
            for metric_name in fold_metrics_list[0].keys():
                if metric_name != "Fold":
                    # Calculate and round the Averages and Std Devs to 3 decimals
                    ...
                    # Extract the individual fold metrics and add them as separate columns in the summary
                    for fold_idx in range(K_FOLDS):
                        if fold_idx < len(fold_metrics_list):
                            avg_metrics[f"Fold{fold_idx+1}_{metric_name}"] = fold_metrics_list[fold_idx][metric_name]
                        else:
                            avg_metrics[f"Fold{fold_idx+1}_{metric_name}"] = None # Leave empty if pruned
