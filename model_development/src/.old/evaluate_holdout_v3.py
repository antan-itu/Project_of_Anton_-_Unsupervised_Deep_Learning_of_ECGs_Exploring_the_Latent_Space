...
def bootstrap_recon_ci(ss_res_arr, ss_tot_arr, elements_per_sample, n_bootstraps=1000):
    # 1. Exact calculation on the full arrays
...
# 4. Evaluation Module
# ================================
def evaluate_subset(X_subset, y_subset, ss_res_subset, ss_tot_subset, xgb_model, lr_model, elements_per_sample, run_dir, file_suffix, subset_mask=None):
    ...
    rmse_str, r2_str = bootstrap_recon_ci(ss_res_subset, ss_tot_subset, elements_per_sample)
    xgb_auc_str = bootstrap_clf_ci(y_subset, xgb_probs, roc_auc_score)
    xgb_prauc_str = bootstrap_clf_ci(y_subset, xgb_probs, average_precision_score)
    lr_auc_str = bootstrap_clf_ci(y_subset, lr_probs, roc_auc_score)
    lr_prauc_str = bootstrap_clf_ci(y_subset, lr_probs, average_precision_score)
