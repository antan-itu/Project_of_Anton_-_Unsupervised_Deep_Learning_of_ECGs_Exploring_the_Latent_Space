...
class ECGDataset:
  def __init__(self, h5_file_path):
      print(f"\nLoading the dataset into RAM from {h5_file_path}...")
...
def calculate_ci(metric_list):
    ...
    return f"[{round(mean_val - margin_of_error, 3):.3f}, {round(mean_val + margin_of_error, 3):.3f}]"
...
def evaluate_latent_classification(model, dataset, train_idx, val_idx, global_labels):
    ...
    metrics = {
        "XGB_AUC": round(xgb_auc, 4), "XGB_PRAUC": round(xgb_prauc, 4), "XGB_Acc": round(xgb_acc, 4),
        "LR_AUC": round(lr_auc, 4), "LR_PRAUC": round(lr_prauc, 4), "LR_Acc": round(lr_acc, 4)
    }
    return metrics, y_val, xgb_probs, lr_probs

def export_val_data_and_generate_plots(model, dataloader, history_dict, plot_dir, run_dir, eval_batches, val_idx, best_clf_data):
  model.eval()
  
  # Base Reconstructions Plot
...
for idx, p in enumerate(EXPERIMENT_COMBINATIONS):
  ...
  for fold in range(K_FOLDS):
      ...
      print("      Extracting latents and running Classifiers (XGBoost & LogReg)...")
      clf_metrics, y_val_clf, xgb_probs, lr_probs = evaluate_latent_classification(model, full_dataset, train_idx, val_idx, y_labels_global)
  ...
  # Automated 95% CI Generation
  ci_val_rmse = calculate_ci([m["Val_RMSE"] for m in fold_metrics_list])
  ci_xgb_auc = calculate_ci([m["XGB_AUC"] for m in fold_metrics_list])
  ci_xgb_prauc = calculate_ci([m["XGB_PRAUC"] for m in fold_metrics_list])
  ci_lr_auc = calculate_ci([m["LR_AUC"] for m in fold_metrics_list])
  ci_lr_prauc = calculate_ci([m["LR_PRAUC"] for m in fold_metrics_list])
  ...
  export_val_data_and_generate_plots(best_model, best_fold_val_loader, best_fold_history, plot_dir, run_dir, plot_eval_batches, best_val_idx, best_clf_data)
